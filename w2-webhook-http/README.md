...
2. Create a **Header Auth** credential and select it on the Webhook node. The workflow ships with `authentication: headerAuth` and no key value — n8n exports credential references, never secrets.
...
*(Build-time issues from the original N8N102 session aren't recorded here — this workflow was built during the course in early August and exported on 8/14. Anything that cost real time then is worth adding.)*
