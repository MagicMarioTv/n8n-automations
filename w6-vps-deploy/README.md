...
2. Buy a domain and add an **A record** pointing a subdomain at that IP — e.g. `n8n` → `198.211.112.8`. Wait for it to propagate (`getent hosts your.subdomain` should return the IP).
3. SSH in, install Docker Engine + the Compose plugin from Docker's own apt repo (not Ubuntu's bundled version — it's older).
...
| *(no `ports:` on `n8n`)* | Deliberate removal. Week 5 published `5678:5678` straight to the browser. Here, Caddy is the only thing the outside world talks to — `n8n` stays reachable only inside Docker's own network, by service name (`n8n:5678`), which Compose wires up automatically between services in the same file. |
...
**Worth separating clearly:** this looked like it could be a deploy problem (wrong response = something's misconfigured), but the JSON coming back at all was actually proof the deploy had already fully succeeded. Reading the message precisely — an application-level complaint about node wiring, not a network or certificate error — was the difference between debugging the right layer and the wrong one.
...
- **Root-only SSH access.** No separate non-root user was created; this build SSHs in as `root` directly. A more hardened setup creates a dedicated sudo user and disables root login entirely. Skipped for time today — worth doing before this box holds anything higher-stakes than a portfolio demo.
...
Weeks 7–8 build Project 1 on top of this box — the first thing this VPS exists to host, rather than a test workflow proving it can.
